...
def main():
    ...
    def get_indices():
        ...
        try:
            ...
            print(f"Error: No response from the marqo endpoint: { MARQO_ENDPOINT } \n")
        ...
        if len(indices["results"]) < 1:
            ...
            print("Error:")
            print("No indices found at the marqo endpoint: {MARQO_ENDPOINT} \n")
        ...
        if len(models["models"]) < 1:
            st.subheader("Error:")
            print(f"No models found at the marqo endpoint: { MARQO_ENDPOINT } \n ")
                ...
                for page in doc:
                    ...
                    for block in blocks:
                        if block[6] == 0:  # that is, if it is a text block
                            ...
                            parag = parag.encode("utf-8", errors="replace").decode(
                                "utf-8"
                            )
        ...
        if numentries < 1:
            ...
            for file in files:
                ...
                if filename.endswith(".pdf"):
                    ...
                    output_file = os.path.join(OUTPUT_DIRECTORY, index, filename)
    ...
    if input("Define the system prompt ? (y/n)") == ("y" or "Y"):
        print(
            "Use the string {BG} to indicate where the background information should be inserted in the prompt, otherwise, background will be appended to the end of the prompt."
        )
            ...
            if num_sources > 0:
                ...
                if lang == "EN":
                    if "{BG}" in system_prompt_en:
                        background = system_prompt_en.replace("{BG}", background)
                    else:
                    ...
                    if "{BG}" in system_prompt_de:
                        background = system_prompt_de.replace("{BG}", background)
                    else:
